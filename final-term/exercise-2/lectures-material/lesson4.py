...
def configurationG(deg):
    ...
    while len(nodes) > 1:
        ...
        if (i % 100000) == 0:
            print(f"The are {str(len(nodes))} in the nodes structure")
...
def GenWS2DG(n, r, k, q):
    ...
    for i in range(n):
        # Strong-ties
        for j in range(i + 1,
                       n):  # we add edge only towards next nodes, since edge to previous nodes have been added when these nodes have been processed
